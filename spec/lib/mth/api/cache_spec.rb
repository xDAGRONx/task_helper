  ...
  describe '#get' do
    context 'call already cached' do
      it 'should query the existing call' do
        call1 = MTH::API::Call.new(route: 'hello')
        call2 = MTH::API::Call.new(route: 'hello')
        expect(MTH::API::Call).to receive(:new).with(route: 'hello')
    ...
    context 'call not yet cached' do
      it 'should query a new call' do
        call1 = MTH::API::Call.new(route: 'hello')
        call2 = MTH::API::Call.new(route: 'goodbye')
        expect(call1).to receive(:run).once
        expect(call2).to receive(:run).once
        expect(MTH::API::Call).to receive(:new).once.ordered
      ...
      it 'should cache the new call' do
        call1 = MTH::API::Call.new(route: 'hello')
        call2 = MTH::API::Call.new(route: 'goodbye')
        call3 = MTH::API::Call.new(route: 'goodbye')
        expect(call1).to receive(:run).once
        expect(call2).to receive(:run).twice
        expect(MTH::API::Call).to receive(:new).once.ordered
          .with(route: 'hello').and_return(call1)
        expect(MTH::API::Call).to receive(:new).twice.ordered
      ...
      it 'should remove older calls if the limit is reached' do
        call1 = MTH::API::Call.new(route: 'hello')
        call2 = MTH::API::Call.new(route: 'goodbye')
        call3 = MTH::API::Call.new(route: 'foobar')
        call4 = MTH::API::Call.new(route: 'hello')
        expect(call1).to receive(:run).once
        expect(call2).to receive(:run).once
